...
def determineSliceLocation(dicomFileName):
    f = pydicom.read_file(dicomFileName, force='True')
...
if __name__ == '__main__':
    if len(sys.argv)<4:
        ...
        imageOutputName = "{0}/Images/{1}.nii.gz".format(outputDir, outputBasename)
        ...
        fileList = [i for i in os.listdir(DICOMDir) if i[-4:]=='.dcm']
        image = []
        imageLocs = []
        struct = []
        dose = []
        other = 0
        for f in fileList:
            c = determineClass("{0}/{1}".format(DICOMDir, f))
            if c=='image':
                image.append("{0}/{1}".format(DICOMDir, f))
                sLoc = determineSliceLocation("{0}/{1}".format(DICOMDir, f))
        ...
        sitk.WriteImage(DICOMImageFile, imageOutputName)
